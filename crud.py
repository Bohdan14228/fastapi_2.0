...
async def get_users_with_posts_and_profiles(
    ...
    stmt = select(User).options(selectinload(User.profile), selectinload(User.posts)).order_by(User.id)
    users = await session.scalars(stmt)
    for user in users:
        print("**" * 10)
...
async def get_orders_with_product(session: AsyncSession) -> list[Order]:
...
async def main_relations(session: AsyncSession):
    ...
    # user_bob = await get_user_buy_username(session=session, username="bob")
...
async def demo_m2m(session: AsyncSession):
    ...
    orders = await get_orders_with_product(session)
    for order in orders:
